...
<body>
  ...
  <form name="formulario16" action="<?php echo htmlspecialchars($_SERVER['PHP_SELF']); ?>" method="post">
    ...
    if (isset($_POST['nombres'])) {
      ...
      //con un bucle foreach se recorre el array y en cada vuelta se va guardando el valor en la variable $nombre
      foreach($_POST['nombres'] as $nombre) {

        /*el valor del input introducido se recoge y para evitar que se vaya mostrando todos los inputs(es decir, para que salgan ocultos) 
        se declara de tipo "hidden"
        */
